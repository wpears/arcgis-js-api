...
a=(1-f)*Math.tan(a);var k=1/Math.sqrt(1+a*a),m=a*k,e=Math.atan2(a,b);a=k*p;for(var l=1-a*a,c=2.7233160610754688E11*l/4.040829998466145E13,t=1+c/16384*(4096+c*(-768+c*(320-175*c))),g=c/1024*(256+c*(-128+c*(74-47*c))),c=d/(6356752.31424518*t),w=2*Math.PI,h,n,q;1E-12<Math.abs(c-w);){q=Math.cos(2*e+c);h=Math.sin(c);n=Math.cos(c);var s=g*h*(q+g/4*(n*(-1+2*q*q)-g/6*q*(-3+4*h*h)*(-3+4*q*q))),w=c,c=d/(6356752.31424518*t)+s}d=m*h-k*n*b;d=Math.atan2(m*n+k*h*b,(1-f)*Math.sqrt(a*a+d*d));p=Math.atan2(h*p,k*n-
m*h*b);b=f/16*l*(4+f*(4-3*l));return new u((r+(p-(1-b)*f*a*(c+b*h*(q+b*n*(-1+2*q*q)))))/(Math.PI/180),d/(Math.PI/180),new H({wkid:4326}))}function z(a,r,b,d){var f=1/298.257223563,p=d-r,k=Math.atan((1-f)*Math.tan(a)),m=Math.atan((1-f)*Math.tan(b)),e=Math.sin(k),k=Math.cos(k),l=Math.sin(m),m=Math.cos(m),c=p,t,g=1E3,w,h,n,q,s;do{n=Math.sin(c);q=Math.cos(c);h=Math.sqrt(m*n*m*n+(k*l-e*m*q)*(k*l-e*m*q));if(0===h)return 0;q=e*l+k*m*q;s=Math.atan2(h,q);var u=k*m*n/h;w=1-u*u;n=q-2*e*l/w;isNaN(n)&&(n=0);var v=
f/16*w*(4+f*(4-3*w));t=c;c=p+(1-v)*f*u*(s+v*h*(n+v*q*(-1+2*n*n)))}while(1E-12<Math.abs(c-t)&&0<--g);if(0===g)return e=6371009*Math.acos(Math.sin(a)*Math.sin(b)+Math.cos(a)*Math.cos(b)*Math.cos(d-r)),l=d-r,k=Math.sin(l)*Math.cos(b),a=Math.cos(a)*Math.sin(b)-Math.sin(a)*Math.cos(b)*Math.cos(l),{azimuth:Math.atan2(k,a),geodesicDistance:e};a=2.7233160610754688E11*w/4.040829998466145E13;b=a/1024*(256+a*(-128+a*(74-47*a)));a=6356752.31424518*(1+a/16384*(4096+a*(-768+a*(320-175*a))))*(s-b*h*(n+b/4*(q*(-1+
2*n*n)-b/6*n*(-3+4*h*h)*(-3+4*n*n))));b=Math.atan2(m*Math.sin(c),k*l-e*m*Math.cos(c));e=Math.atan2(k*Math.sin(c),k*l*Math.cos(c)-e*m);return{azimuth:b,geodesicDistance:a,reverseAzimuth:e}}function D(a,r){var b=Math.PI/180;637.100877151506>r&&(r=637.100877151506);if(!(a instanceof y||a instanceof C))throw console.error("_geodesicDensify: the input geometry is neither polyline nor polygon"),Error("_geodesicDensify: the input geometry is neither polyline nor polygon");var d=a instanceof y,f=[],p;s.forEach(d?
a.paths:a.rings,function(a){f.push(p=[]);p.push([a[0][0],a[0][1]]);var m,e,l,c,t,g;m=a[0][0]*b;e=a[0][1]*b;for(t=0;t<a.length-1;t++){l=a[t+1][0]*b;c=a[t+1][1]*b;c=z(e,m,c,l);l=c.azimuth;c=c.geodesicDistance;var d=c/r;if(1<d){for(g=1;g<=d-1;g++){var h=x(e,m,l,g*r);p.push([h.x,h.y])}g=x(e,m,l,(c+Math.floor(d-1)*r)/2);p.push([g.x,g.y])}e=x(e,m,l,c);p.push([e.x,e.y]);m=e.x*b;e=e.y*b}});return d?new y({paths:f,spatialReference:a.spatialReference}):new C({rings:f,spatialReference:a.spatialReference})}var A=
